...
class ActionConstructorForm extends AbstractIdeForm
{
    ...
    protected function listCellFactory(UXListCell $cell, Action $action = null, $empty = false)
    {
        if ($action) {
    ...
    {
        $group = $this->editor->getManager()->findGroup($actionType->getGroup());

        if (!$group) {
            throw new IdeException("Failed to find action group {$actionType->getGroup()}");
        }

        $groupCode = $group->getCode();
        $tab = $buildTabs[$groupCode];
        ...
        if (!$tab) {
            ...
            $buildTabs[$groupCode] = $tab;
        }

        if (!$subGroups[$groupCode][$subGroup]) {
            if ($subGroup) {
                $subGroupName = $group->getSubGroupName($subGroup);

                $label = _(new UXLabel($subGroupName));
                $label->font = UXFont::of($label->font, $label->font->size, 'BOLD');

                if ($subGroups[$groupCode]) {
                    $label->paddingTop = 5;
                }
