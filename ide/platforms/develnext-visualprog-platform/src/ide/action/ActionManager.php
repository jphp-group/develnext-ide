...
class ActionManager
{
     ...
     * @var AbstractActionTypeGroup[]
     */
    protected array $actionGroupByCode = [];

    /**
     * @var AbstractActionType[]
     */
    protected array $actionTypeByTagName = [];
    /**
     * @var int
     */
    protected int $lastUpdated = 0;

    public static function get(): ?ActionManager
    {
        $visprog = Project::findSupportOfCurrent('visprog');
        if ($visprog) {
            return $visprog->getActionManager(Ide::project());
        } else {
            return null;
        }
    }
    ...
    /**
     * @param string $code
     * @return AbstractActionTypeGroup
     */
    public function findGroup(string $code): ?AbstractActionTypeGroup
    {
        return $this->actionGroupByCode[$code];
    }
    ...
    /**
     * @param AbstractActionType|AbstractActionTypeGroup|string $type
     * @throws IdeException
     */
    public function registerType($type)
    {
        ...
        } else if ($type instanceof AbstractActionTypeGroup) {
            $this->actionGroups[str::lower(reflect::typeOf($type))] = $type;
            $this->actionGroupByCode[$type->getCode()] = $type;
            $this->lastUpdated = Time::millis();
    ...
    public function unregisterType($class)
    {
        ...
        if ($type = $this->actionTypes[$class]) {
            ...
            $this->lastUpdated = Time::millis();
        } else if ($group = $this->actionGroups[$class]) {
            unset($this->actionGroups[$group]);
            unset($this->actionGroupByCode[$group->getCode()]);
            $this->lastUpdated = Time::millis();
        } else {
